...
ThreadPool::ThreadPool(size_t num_threads, size_t queue_size, const std::string &pool_name)
    ...
    debug::LogBufferManager::getInstance().appendTo(
        "ThreadPool",
        "Creating ThreadPool '" + pool_name_ + "' with " + std::to_string(num_threads) + " threads",
...
void ThreadPool::shutdown() {
    ...
    debug::LogBufferManager::getInstance().appendTo(
        "ThreadPool",
        "ThreadPool '" + pool_name_ + "' shut down",
        debug::LogContext::Debug);
...
std::future<void> ThreadPool::shutdown_async() {
    ...
    std::thread shutdown_thread([this, promise = std::move(shutdown_promise)]() mutable {
    ...
    while (!shutdown_.load(std::memory_order_acquire)) {
        ...
        if (tasks_.pop(task, task_id, task_name)) {
            try {
                ...
                debug::LogBufferManager::getInstance().appendTo(
                    "ThreadPool",
                    "Exception in ThreadPool '" + pool_name_ + "' worker: " + e.what(),
