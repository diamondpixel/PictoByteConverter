...
public:
    ...
    explicit SpillableQueue(size_t max_memory_items = 1000, 
        ...
        printDebug("Creating SpillableQueue '" + queue_name_ + "' with max in-memory items: " + 
            ...
            try {
                if (!std::filesystem::exists(spill_directory_path_)) {
                    if (!std::filesystem::create_directories(spill_directory_path_)) {
                        printError("SpillableQueue '" + queue_name_ + "': Failed to create spill directory: " + spill_directory_path_);
                        spill_directory_path_.clear();
                    }
                } else if (!std::filesystem::is_directory(spill_directory_path_)) {
                    printError("SpillableQueue '" + queue_name_ + "': Spill path exists but is not a directory: " + spill_directory_path_);
                    spill_directory_path_.clear();
                }
            } catch (const std::filesystem::filesystem_error &e) {
                printError("SpillableQueue '" + queue_name_ + "': Filesystem error with spill directory '" + 
        ...
        if (!spill_directory_path_.empty()) {
            ...
            while (!spilled_task_files_.empty()) {
                ...
                try {
                    ...
                    printWarning("SpillableQueue '" + queue_name_ + "': Error deleting spill file '" + 
                                file_to_delete + "': " + e.what());
                }
            }
        }
        
        // Log destruction
        printDebug("SpillableQueue '" + queue_name_ + "' destroyed");
    }
    ...
    bool push(const T &item) {
        ...
        { // Scope for the first lock
            std::lock_guard<std::mutex> lock(mutex_);
            if (shutdown_flag_) {
                printWarning("SpillableQueue '" + queue_name_ + "': Attempted to push to a shutting down queue.");
                return false;
            }
            ...
            if (queue_.size() >= max_in_memory_items_ && !spill_directory_path_.empty()) {
                ...
                printDebug("SpillableQueue '" + queue_name_ + "': Spilling item to disk: " + filename_stream.str());
            } else {
        ...
        if (spill_filename_optional && item_to_serialize_optional) {
            ...
            try {
                ...
                if (!file) {
                    printError("SpillableQueue '" + queue_name_ + "': Failed to open spill file for writing: " + filename);
                    ...
                    if (!item_to_serialize_optional.value().serialize(file)) {
                        printError("SpillableQueue '" + queue_name_ + "': Failed to serialize item to spill file: " + filename);
                        file.close(); // Close before removing
                        try { 
                            if (std::filesystem::exists(filename)) 
                                std::filesystem::remove(filename); 
                        } catch (const std::filesystem::filesystem_error& e) { 
                            printWarning("SpillableQueue '" + queue_name_ + "': push: Filesystem error removing failed spill file '" + 
                                        filename + "': " + e.what()); 
                        } catch (...) {}
                    } else {
                ...
                printError("SpillableQueue '" + queue_name_ + "': Exception while spilling item to disk ('" + 
                          filename + "'): " + std::string(e.what()));
                try { 
                    if (std::filesystem::exists(filename)) 
                        std::filesystem::remove(filename); 
                } catch (const std::filesystem::filesystem_error& e) { 
                    printWarning("SpillableQueue '" + queue_name_ + "': push: Filesystem error removing failed spill file (exception) '" + 
                                filename + "': " + e.what()); 
                } catch (...) {}
            }
            ...
            if (serialization_succeeded) {
                ...
                if (shutdown_flag_) { // Check shutdown again, in case it happened during serialization
                    printWarning("SpillableQueue '" + queue_name_ + "': Queue shutdown during spill operation for " + 
                                filename + ". Cleaning up.");
                    try { 
                        if (std::filesystem::exists(filename)) 
                            std::filesystem::remove(filename); 
                    } catch (const std::filesystem::filesystem_error& e) { 
                        printWarning("SpillableQueue '" + queue_name_ + "': push: Filesystem error removing spill file during shutdown '" + 
                                    filename + "': " + e.what()); 
                    } catch(...) {}
                    return false;
                } else {
    ...
    bool try_push(const T &item) {
        ...
        if (shutdown_flag_) {
            printWarning("SpillableQueue '" + queue_name_ + "': Attempted to try_push to a shutting down queue.");
            return false;
        }
    ...
    bool pop(T &item) {
        ...
        if (!spilled_task_files_.empty()) {
            ...
            try {
                ...
                if (!file) {
                    printError("SpillableQueue '" + queue_name_ + "': Failed to open spill file for reading: " + filename);
                } else {
                    if (!item.deserialize(file)) {
                        printError("SpillableQueue '" + queue_name_ + "': Failed to deserialize item from spill file: " + filename);
                    } else {
                        deserialization_succeeded = true;
                    }
                }
            } catch (const std::exception &e) {
                printError("SpillableQueue '" + queue_name_ + "': Exception while loading item from disk ('" + 
                          filename + "'): " + std::string(e.what()));
            }
            ...
            try {
                ...
                printWarning("SpillableQueue '" + queue_name_ + "': Error deleting spill file after read '" + 
                            filename + "': " + e.what());
            }
    ...
    void shutdown() {
        ...
        printDebug("SpillableQueue '" + queue_name_ + "' marked as done");
    }
