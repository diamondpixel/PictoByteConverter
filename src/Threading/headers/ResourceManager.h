...
private:
    ...
    std::string thread_id_to_string(const std::thread::id& id) const {
        std::stringstream ss;
        ss << id;
        return ss.str();
    }
...
public:
    ...
    static std::string formatDuration(uint64_t nanoseconds) {
        ...
        if (nanoseconds < 1000) {
            ...
            oss << std::fixed << std::setprecision(2) << (nanoseconds / 1000.0) << " µs";
        } else if (nanoseconds < 1000 * 1000 * 1000) {
            oss << std::fixed << std::setprecision(2) << (nanoseconds / (1000.0 * 1000.0)) << " ms";
        } else {
            oss << std::fixed << std::setprecision(2) << (nanoseconds / (1000.0 * 1000.0 * 1000.0)) << " s";
        }
        return oss.str();
    }
...
public:
    ...
    void registerThread(const std::string& pool_name) {
        ...
        if (it == thread_metrics.end()) {
            ...
            debugLog("Thread " + thread_id_to_string(thread_id) + " registered with pool '" + pool_name + 
                    "'. Active threads: " + std::to_string(active_threads));
        } else {
            ...
            debugLog("Thread " + thread_id_to_string(thread_id) + " updated to pool '" + pool_name + 
                    "'. Active threads: " + std::to_string(active_threads));
        }
    }
    ...
    void unregisterThread() {
        ...
        if (it != thread_metrics.end()) {
            ...
            debugLog("Thread " + thread_id_to_string(thread_id) + " unregistered from pool '" + pool_name + 
                    "'. Lifetime: " + std::to_string(thread_lifetime_ms) + " ms, Tasks: " + 
                    std::to_string(tasks_completed) + ", Avg execution: " + avg_execution_time + 
                    ". Active threads: " + std::to_string(active_threads));
            
            // Notify any waiting threads
            thread_cv.notify_all();
        } else {
            debugLog("Warning: unregisterThread called for unregistered thread " + 
    ...
    void acquireThread(uint64_t task_id, const std::string& task_name) {
        ...
        if (it != thread_metrics.end()) {
            ...
            if (task_it != active_tasks.end()) {
                ...
                if (getDebugMode()) {
                    printDebug("ResourceManager: Thread " + thread_id_to_string(thread_id) + 
                              " acquired for task '" + task_name + "' (ID: " + std::to_string(task_id) + 
                              "). Wait time: " + formatDuration(wait_time));
                }
            } else {
                if (getDebugMode()) {
                    printDebug("ResourceManager: Thread " + thread_id_to_string(thread_id) + 
                              " acquired for task '" + task_name + "' (ID: " + std::to_string(task_id) + 
                              ") but task not found in active tasks");
                }
            }
        } else {
            if (getDebugMode()) {
                printDebug("ResourceManager: Warning - acquireThread called for unregistered thread " + 
                          thread_id_to_string(thread_id));
            }
        }
    }
        ...
        if (it != thread_metrics.end()) {
            ...
            if (it->second.is_executing) {
                ...
                if (task_it != active_tasks.end()) {
                    ...
                    if (getDebugMode()) {
                        printDebug("ResourceManager: Thread " + thread_id_to_string(thread_id) + 
                                  " released after task '" + it->second.current_task_name + "' (ID: " + 
                                  std::to_string(task_id) + "). Execution time: " + formatDuration(execution_time));
                    }
                } else {
                    if (getDebugMode()) {
                        printDebug("ResourceManager: Thread " + thread_id_to_string(thread_id) + 
                                  " released but task ID " + std::to_string(task_id) + 
                                  " not found in active tasks");
                    }
                }
            } else {
                if (getDebugMode()) {
                    printDebug("ResourceManager: Warning - releaseThread called for non-executing thread " + 
                              thread_id_to_string(thread_id));
                }
            }
        } else {
            if (getDebugMode()) {
                printDebug("ResourceManager: Warning - releaseThread called for unregistered thread " + 
                          thread_id_to_string(thread_id));
            }
        }
    }
    ...
    void recordTaskEnqueue(uint64_t task_id, const std::string& task_name, const std::string& pool_name) {
        ...
        if (getDebugMode()) {
            printDebug("ResourceManager: Task '" + task_name + "' (ID: " + std::to_string(task_id) + 
                      ") enqueued in pool '" + pool_name + "'");
        }
    }
            ...
            if (metrics.pool_name == pool_name) {
                ...
                if (metrics.tasks_completed.load() > 0) {
                    ...
                    result.max_execution_time_ns = std::max(result.max_execution_time_ns, 
                                                           static_cast<uint64_t>(metrics.max_execution_time_ns.load()));
                    
                    // Handle min execution time
                    uint64_t min_exec = metrics.min_execution_time_ns.load();
                    if (min_exec < UINT64_MAX) {  // Check if it's been set
                    ...
                    if (metrics.total_wait_time_ns.load() > 0) {
                        ...
                        if (min_wait < UINT64_MAX) {  // Check if it's been set
    ...
    std::vector<TaskMetrics> getRecentTasks(size_t max_tasks = 100) {
        ...
        size_t start_idx = completed_tasks.size() > max_tasks ? 
                          completed_tasks.size() - max_tasks : 0;
     ...
     * @param tag Optional tag for categorizing memory usage
     */
    void trackMemory(size_t size, const std::string& tag = "general") {
        std::lock_guard<std::mutex> lock(memory_usage_mutex);
        memory_usage += size;
        
        // Update tagged memory usage
        memory_usage_by_tag[tag] += size;
        
        if (getDebugMode()) {
            printDebug("ResourceManager: Memory tracked: " + std::to_string(size) + 
                      " bytes for '" + tag + "'. Total: " + std::to_string(memory_usage) + " bytes");
        }
    }
    ...
    void releaseMemory(size_t size, const std::string& tag = "general") {
        ...
        if (getDebugMode()) {
            printDebug("ResourceManager: Memory released: " + std::to_string(size) + 
                      " bytes for '" + tag + "'. Total: " + std::to_string(memory_usage) + " bytes");
        }
    }
